...
_Slide 2_<br />
The human platelet samples were prepared by Dr. Ashok Reddy and analyzed at Thermo by Joshua Nicklay. The sample was from a pre-run normalization step that was not part of the full biological experiment. The new data was collected on an Eclipse Oribtrap Tribrid using the SPS MS3 method in combination with real-time search (RTS) and with a FAIMS front end. Sample loading and a 2-hour reverse phase gradient was used to mimic original data generated at OHSU using our older Thermo Fusion Tribrid instrument. The new data was a simple SPS MS3 run (similar to what the Fusion can do). There was also a run using the real time search acquisition mode (RTS) to see if better sensitivity could be obtained. Both plain SPS MS3 and RTS SPS MS3 were also done with and without front end FAIMS filtering (that data is not presented here).

So what RTS scheme is Thermo using for this data? It is Comet-based with a fixed PSM validator type of cutoff test instead of the more complicated running FDR testing of a linear discriminant function used in the second RTS paper. The database is UniProt human (not sure which one: Swiss-Prot reviewed, Swiss-Prot reviewed with annotated isoforms, canonical reference proteome, canonical reference proteome with isoforms, or the other reference proteome) with trypsin cleavage and a maximum of one missed cleavage (are di-basic motifs excluded from the missed cleavage counting? They should be). Alkylation of Cys and TMT tags on N-term and Lys are specified as static modifications. Oxidized Met is a variable modification. The fixed PSM score testing is XCorr >= 1, DeltaCN >= 0.1 and deltamass less than +/- 10 ppm. You can read more about the Thermo implementation of RTS [here](https://assets.thermofisher.com/TFS-Assets/CMD/posters/po-65536-orbitrap-tribrid-ms-real-time-search-hupo2019-po65536-en.pdf?elqTrackId=a90b29c09abb4b7887666d5d7022a97e&elqaid=4854&elqat=2). Ironically, 10 ppm searching can create difficulties for computing DeltaCN because of too few candidate peptides.
...
> I did some more digging into the RTS high reject number (scans with no or low ions). Of the 2,341 scans with reporter ions below the average cutoff of 500, 2,106 MS2 scans did not have corresponding MS3 scans. The 235 scans with some non-zero reporter ions that failed the average cutoff test are only 2.1% of the scans. The reject rate, if MS2 scans without MS3 scans are ignored, was 9.5% for the Fusion SPS MS3, 3.7% for the Eclipse SPS MS3, and just 2.1% for the Eclipse SPS MS3 with RTS.

Not all scans with reporter ions are usable for quantification in shotgun experiments. Peptides that map to multiple proteins (in the proper context) have ambiguous information content and cannot be used. We routinely use minimally redundant UniProt canonical reference protein databases and employ an extended parsimony protein grouping step to maximize the usable peptides for quantification. The last column of the bottom table are the final numbers and the most accurate for comparisons between platform/methods.
...
_Slide 40_<br />
We know from many of the previous slides that these 9 biological samples are not very different from each other. We can average the intensities for all nine samples for each platform/method and compare that by platform/method (this gives us nice large plots). This is pretty amazing. We have data from two different Orbitrap Tribrid instruments (two generations apart). We have two labs (OHSU and Thermo San Jose), we have two LC systems, we have two different technicians, we have two different dates, and we have two different data acquisition methods (for the Eclipse instrument). The Eclipse data is superb. The Fusion data has some warts but is still surprisingly good. The Fusion was a pretty successful first-generation instrument.

The degree of complexity in these instruments is quite high. Adding the additional complexity of the real time search to the complexity of the Tribrids did not give me much confidence before starting this analysis. I have to say that I think the real time search works well. This sample, where we see 750-900 proteins out of more than 3,000 detectible in a scaled up multi-dimensional LC separation, might seem like a good sample to showcase RTS. There are many more peptides in the sample than the instrument has time to sample. Skipping time-wasting SPS MS3 scans of low quality MS2 should free up the instrument to sample more peptides. Maybe some more thinking is needed to figure out what are the situations where RTS might be worth the biased data acquisition. It is often the case that when you sample more deeply, many of the additional PSMs are low abundance peptides from medium and higher abundance proteins. This might improve the quantitative data quality more than expanding the quantitative profiling depth. RTS for this sample (as configured) was far short of a factor of 2 improvement (it was about 9% better). I also do not think a 1-hour LC run with RTS would have been close to the 2-hour run without RTS.    
